 ...
 *
 */
public class MapActivity<MainActivity> extends Activity implements MapEventsReceiver, LocationListener, SensorEventListener, OnInitListener {
...
//		});	
	...
	public void handleSearchButton(int index, int editResId){
		...
		try {
			...
			if(locationAddress.equalsIgnoreCase("My Location"))
			{
			    ...
			    String addressDisplayName = getAddress(myLocationOverlay.getLocation());
				
				if (sIndex == START_INDEX){
					startPoint = myLocationOverlay.getLocation();//new GeoPoint(address.getLatitude(), address.getLongitude());
					markerStart = updateItineraryMarker(markerStart, startPoint, START_INDEX,
						R.string.departure, R.drawable.marker_departure, -1, addressDisplayName);
					map.getController().setCenter(startPoint);
				} else if (sIndex == DEST_INDEX){
					destinationPoint = myLocationOverlay.getLocation();//new GeoPoint(address.getLatitude(), address.getLongitude());
					markerDestination = updateItineraryMarker(markerDestination, destinationPoint, DEST_INDEX,
						R.string.destination, R.drawable.marker_destination, -1, addressDisplayName);
					map.getController().setCenter(destinationPoint);
				}
		    ...
		    foundAdresses = geocoder.getFromLocationName(locationAddress, 5, 
			...
			if (foundAdresses.size() == 0) { //if no address found, display an error
	            ...
	            final ArrayAdapter<String> addressDisplayNames = new ArrayAdapter<String>(MapActivity.this,android.R.layout.select_dialog_item);
	            for (Address a : foundAdresses)
					addressDisplayNames.add(a.getExtras().getString("display_name"));
	...
	void getKMLFromOverpass(String query){
		/*OverpassAPIProvider overpassProvider = new OverpassAPIProvider();
		String oUrl = overpassProvider.urlForAmenitySearch(query, map.getBoundingBox(), 500, 30);
		HashMap<Long, Element> elements = null;
		/*boolean result = overpassProvider.addInKmlFolder(mKmlDocument.mKmlRoot, elements);// addInKmlFolder(mKmlDocument.mKmlRoot, oUrl);
		/*if (!result)
